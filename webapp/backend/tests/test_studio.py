...
from app.studio import WorkbenchRequest, panorama, run_workbench
...
class StudioTests(unittest.TestCase):
    ...
    def test_panorama_drivers_reconcile_with_annual_change(self) -> None:
        result = panorama(repository, 2019, 2024, None, None, 2022, "reimbursed")
        driver_delta = sum(float(driver["delta"] or 0) for driver in result["drivers"])
        reimbursement = next(kpi for kpi in result["kpis"] if kpi["key"] == "reimbursed")

        self.assert_amount_equal(driver_delta, reimbursement["delta"])
        self.assertEqual(result["context"]["reference_year"], 2022)
        self.assertEqual(result["reliability"]["consolidated_through"], 2024)

    def test_panorama_exposes_weighted_annual_averages(self) -> None:
        result = panorama(repository, 2019, 2024, None, None, 2023, "reimbursed")
        annual_2024 = next(row for row in result["annual"] if row["year"] == 2024)
        direct = repository.query(
            """SELECT SUM(rem)::DOUBLE / NULLIF(SUM(qte), 0) AS average_reimbursed,
                      SUM(dep)::DOUBLE / NULLIF(SUM(qte), 0) AS average_expense
               FROM cube WHERE soi_ann = 2024"""
        )[0]

        self.assert_amount_equal(annual_2024["average_reimbursed"], float(direct["average_reimbursed"]))
        self.assert_amount_equal(annual_2024["average_expense"], float(direct["average_expense"]))
