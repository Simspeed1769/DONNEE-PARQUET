/** L'écran avancé des croisements : « Lien » et « Modèle ».
 *
 *  **Ce module n'est plus routé.** Croisements n'expose que le mode Guidé, qui
 *  accompagne la question du début à la fin ; Lien demandait de choisir un
 *  indicateur de chaque côté sans dire lequel correspond à quoi, et Modèle est
 *  un établi complet fait pour qui sait déjà ce qu'il veut y poser. Trois
 *  portes d'entrée pour une seule question, c'était deux de trop.
 *
 *  Le code reste ici, entier et compilé, parce que rien ne dit qu'il ne
 *  reviendra pas : les endpoints de corrélation qu'il appelle sont eux aussi
 *  toujours servis. Pour le rebrancher, il suffit de rendre `<AdvancedCross />`
 *  depuis `pages/CorrelationsPage.tsx`.
 *
 *  L'ordre y est celui de la version précédente : la conclusion en français
 *  d'abord, avec l'échelle de force qui montre l'estimation *et* son
 *  incertitude, puis le nuage, puis le détail statistique, replié mais entier.
 *  L'intervalle de confiance y est traité comme le résultat principal : sur
 *  douze régions il traverse presque toujours zéro, et c'est précisément ce
 *  qu'il faut voir.
 */
...
type Side = "x" | "y";
/** Deux questions : l'une répond à « est-ce que ceci va avec cela ? »,
 *  l'autre à « une fois le reste tenu constant, que pèse chaque variable ? ».
 *  Toutes deux partagent le périmètre — même unité, même période, même
 *  population. */
...
export function AdvancedCross({ onOpenMethodology }: Props) {
