...
echo [2/5] Installation de l'interface...
call npm install --prefix "app\frontend"
if errorlevel 1 goto :error

echo [3/5] Construction de l'interface...
call npm run build --prefix "app\frontend"
if errorlevel 1 goto :error

echo [4/5] Construction du cube compact (quelques minutes)...
...
echo [5/5] Construction du cube des reglements...
if exist "cube_parts3\main_2025.parquet" (
    "app\backend\.venv\Scripts\python.exe" "tools\build_cube_reglement.py"
    if errorlevel 1 goto :error
) else (
    echo   Tranches de flux absentes : etape ignoree, lecture en annee de reglement limitee au remboursement.
)
